...
![Консоль Houdini](images/example_console.png)
...
*   **📊 Статистика времени**:
    ...
    *   *Примечание: При включенном режиме "Render All Frames with a Single Process" статистика по кадрам и время до окончания рендера пока! не выводится*
...
### 5. Ограничения режима "Single Process"
Если в USD ROP (Karma) включена опция **"Render All Frames with a Single Process"**:
*   **Консоль**: Вместо прогноза времени будет писать `Генерация сцены... (Single Process Mode)`.
*   **Статистика**: В итоговом отчете Telegram будут скрыты *Мин/Макс время кадра*, так как рендер идет сплошным потоком и индивидуальное время кадров неизвестно. Будет доступно только *Среднее время*.
*   **Прогресс**: Полноценный прогресс-бар (покадровый) в этом режиме невозможен средствами Python-скриптов.

Чтобы видеть детальную статистику по каждому кадру, отключите эту опцию (но рендер может стать чуть медленнее из-за перезапуска процессов).
